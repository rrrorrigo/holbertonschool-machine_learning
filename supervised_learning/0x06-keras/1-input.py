...
def build_model(nx, layers, activations, lambtha, keep_prob):
  """Function that build a keras model
    ...
    lyr = K.layers.Dense(layers[0], activation=activations[0], kernel_regularizer=regularizer)(inputLayer)
    for i in range(1, len(layers)):
        dropoutLayer = K.layers.Dropout(1 - keep_prob)(lyr)
        lyr = K.layers.Dense(layers[i], activation=activations[i], kernel_regularizer=regularizer)(dropoutLayer)
