...
class Yolo:
    ...
    def predict(self, folder_path):
        """Function that predict image detection of image in folder_path
        ...
        image_list, images_path = self.load_images(folder_path)
        pimages, images_shapes = self.preprocess_images(image_list)
        all_outputs = self.model.predict(pimages)

        prediction = []

        for i in range(pimages.shape[0]):
            output = [o[i] for o in all_outputs]
            boxes, box_conf, box_probs = self.process_outputs(
                output, images_shapes[i]
            )
            ...
            pred_box, pred_class, pred_score = self.non_max_suppression(
                f_boxes, f_classes, f_scores
            )

            f = images_path[i].split('/')[-1]

            prediction.append((pred_box, pred_class, pred_score))

            self.show_boxes(image_list[i], pred_box, pred_class, pred_score, f)

        return prediction, images_path
